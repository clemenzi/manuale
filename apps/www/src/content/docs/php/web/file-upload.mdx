...
`$_FILES` contiene informazioni come nome originale, tipo, dimensione, percorso temporaneo ed eventuali errori.
...
Per il tipo, non fidarti solo del nome del file. Usa controlli server-side e accetta soltanto i formati previsti.
...
`move_uploaded_file` sposta il file dalla cartella temporanea a quella scelta.
...
`__DIR__` indica la cartella del file PHP corrente. Così costruisci un percorso stabile.

## Evitare nomi pericolosi

Il nome originale arriva dal computer dell'utente, quindi non va usato senza controlli.
...
`uniqid` crea un nome abbastanza unico da evitare sovrascritture accidentali. In un progetto reale controlleresti anche il tipo effettivo del file.

## Checklist minima

Prima di salvare un file caricato, verifica sempre:
