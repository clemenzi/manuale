...
`GROUP BY` serve quando vuoi **raggruppare** le righe che condividono uno stesso valore.
...
`GROUP BY` applica questa logica ai dati della tabella.
...
Per questo `GROUP BY` è comune nei report.
...
Prima raggruppi per città, poi per stato all'interno di ogni città.
