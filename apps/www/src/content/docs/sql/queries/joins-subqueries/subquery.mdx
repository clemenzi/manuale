...
Serve quando la risposta finale dipende da un passaggio intermedio: prima calcoli un risultato, poi lo usi nella query principale.
...
Quando la query interna restituisce più valori, puoi usare `IN`.
...
Se le subquery sono troppo annidate, una CTE con `WITH` può rendere la query più leggibile. La trovi nella pagina dedicata.
