...
`UPDATE` serve quando devi cambiare un dato già presente.
...
La parte da controllare con più attenzione è `WHERE`.

```sql
WHERE id = 1
```

Questa condizione sceglie la riga da modificare. Senza di essa, il database può aggiornare tutte le righe.
...
Anche in questo caso il filtro `WHERE` decide quali righe cambiano.
