...
Non inserisci ancora dati. Definisci il nome della tabella, le colonne e alcune regole di base.

Puoi pensarlo come un modulo: stabilisci quali campi servono, per esempio nome, email e data di iscrizione.
...
Se riesci a descrivere la tabella in questo modo, la definizione è comprensibile.

## La chiave primaria

`PRIMARY KEY` indica la **chiave primaria**, cioè il valore che identifica una riga senza confonderla con altre.

È come il numero di tessera di una palestra: due persone possono chiamarsi entrambe Luca, ma non dovrebbero avere lo stesso numero.
...
Il database conserva i dati e applica queste regole per mantenerli coerenti.

## Scegliere colonne chiare

All’inizio può venire voglia di creare colonne generiche come `info` o `dati`.

È meglio evitarlo. Una tabella è più comprensibile quando ogni colonna ha un compito chiaro.
...
Non serve progettare tutto perfettamente al primo colpo, ma queste domande evitano molte tabelle confuse.
