...
I due esempi producono lo stesso risultato.

All'inizio il ciclo `for-each` e spesso più facile da leggere. La lambda diventa utile quando usi metodi che vogliono ricevere una piccola azione, come `forEach`, `sort`, `filter` o `map`.
...
Una lambda può ricevere anche più valori.
...
Evita lambda troppo lunghe. Se contengono molte righe, spesso è meglio creare un metodo con un nome chiaro.
