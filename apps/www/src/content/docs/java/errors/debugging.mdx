...
Non significa solo "togliere errori": significa capire che cosa fa davvero il programma, passo dopo passo.

## Leggere prima il messaggio

Quando Java mostra un errore, non cambiare subito codice a caso.
...
Un modo semplice per capire cosa succede è stampare valori.
...
Il programma fallisce. Il problema è nella condizione:

```java
i <= voti.length
```

L'ultimo indice valido è `voti.length - 1`.
