...
## Perché c'è try catch

Scrivere su un file può fallire: la cartella potrebbe non esistere, il file potrebbe essere bloccato oppure potresti non avere i permessi.

Java ti chiede di gestire questa possibilità.
...
Di solito `FileWriter` sovrascrive il file. Per aggiungere testo alla fine, usa il secondo parametro `true`:
...
I file permettono a Java di leggere e salvare dati esterni al codice.
