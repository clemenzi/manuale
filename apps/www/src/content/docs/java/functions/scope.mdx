...
Una variabile non è visibile ovunque: di solito esiste solo nel blocco in cui è stata creata.
...
Anche un blocco `if` o `for` crea uno scope.
...
`numero` nasce dentro il blocco `if` e non è più visibile alla sua fine.
...
Questo evita che i contatori temporanei restino visibili nel resto del programma.
...
Quando Java dice che una variabile "cannot be resolved", spesso stai provando a usarla fuori dal suo scope.
