...
Per un programma piccolo può bastare un file `.cpp`. In un progetto con molte funzioni, separare il codice rende più facile trovare e modificare ogni parte.

Dividere il codice in più file assegna a ogni file uno scopo preciso.
...
L'header espone ciò che gli altri file possono usare; il `.cpp` contiene le definizioni.
...
**Regola pratica:** se qualcuno ha bisogno di _sapere che esiste_, mettilo nell'header. Se qualcuno ha bisogno di _vedere come funziona_, mettilo nel `.cpp`.
