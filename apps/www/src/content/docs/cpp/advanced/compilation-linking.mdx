...
Quando compili un programma C++, il sistema esegue tre fasi distinte.

Conoscere queste fasi aiuta a:
...
Il preprocessore elabora il tuo codice _prima_ della compilazione. Sostituisce ogni `#include` con il contenuto del file incluso, espande le macro e gestisce i blocchi `#ifdef`.
...
Questo comando esegue le tre fasi per ogni file `.cpp` e poi collega i risultati.

## I flag di g++ più utili

I **flag** sono opzioni che modificano il comportamento del comando e iniziano con `-`.
...
### Ottimizzazione: rendere il programma più veloce
...
Oppure in due fasi (utile nei progetti grandi, perché ricompili solo i file modificati):
...
Non è necessario imparare subito ogni dettaglio di un Makefile; basta sapere a cosa serve.
