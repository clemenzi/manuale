...
Per rappresentare i giorni della settimana potresti usare numeri, ma il loro significato non sarebbe immediato.

Un'**enumerazione** (`enum`) assegna nomi a un insieme fisso di valori. Al posto di `3`, puoi scrivere `GIOVEDI`.
...
`enum` e `switch` si combinano bene quando ogni valore richiede un'azione diversa:
...
Con `enum class` devi scrivere `NomeEnum::VALORE`; i valori restano così separati da quelli di altre enumerazioni. È la scelta preferita nel C++ moderno.
