...
Un oggetto contiene dati e può eseguire operazioni su quei dati. I **metodi** definiscono queste operazioni.

Un oggetto `Cane` può contenere nome, razza ed età e offrire metodi come abbaiare. Un `ContoBancario` può gestire depositi e prelievi.

Un metodo è una **funzione definita dentro una classe**.
...
Se un metodo legge i dati senza modificarli, aggiungi `const` alla fine della dichiarazione. Il compilatore potrà verificare questo vincolo:
