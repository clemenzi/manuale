...
Le classi `Cane`, `Gatto` e `Coniglio` possono condividere nome, età e operazioni come mangiare e dormire.

Con l'**ereditarietà** definisci questi elementi in `Animale`; `Cane`, `Gatto` e `Coniglio` li ereditano e aggiungono quelli specifici.

Un `Cane` è quindi un `Animale` con caratteristiche aggiuntive.
...
Quando crei un oggetto `Cane`, C++ chiama prima il costruttore di `Animale` e poi quello di `Cane`. La lista di inizializzazione passa i dati alla classe base:
