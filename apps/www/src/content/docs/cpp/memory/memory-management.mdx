...
La memoria dinamica serve quando la quantità di memoria necessaria si conosce solo durante l'esecuzione.

Con la **memoria dinamica** allochi la quantità necessaria a runtime e la liberi quando hai finito.
...
La memoria allocata nell'heap resta occupata finché non la liberi o finché il processo non termina.
...
Gestire `new` e `delete` manualmente espone a errori. In C++ moderno si usano i **puntatori smart**, che liberano la memoria automaticamente:
