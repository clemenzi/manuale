...
Un **riferimento** è un secondo nome per una variabile già esistente.

Se modifichi il valore tramite il riferimento, modifichi la variabile originale.

Sono utili soprattutto per passare variabili alle funzioni senza copiarle.
...
Se passi una stringa o un oggetto grande a una funzione, il computer ne fa una copia. Uno spreco di tempo e memoria. Con `const &` passi il riferimento (nessuna copia) ma impedisci le modifiche:
