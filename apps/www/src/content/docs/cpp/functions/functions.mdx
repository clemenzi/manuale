...
Se devi calcolare l'area di un cerchio in più punti, ripetere la formula rende ogni modifica più difficile.

Le **funzioni** raccolgono il codice in un punto, gli danno un nome e permettono di richiamarlo quando serve.
...
Una volta definita, puoi eseguirla scrivendo il suo nome seguito dalle parentesi:
...
Quando il programma esegue `return`, la funzione termina e restituisce il valore indicato.
...
int main() {
    saluta("Alice");               // usa il default e stampa: Ciao, Alice!
    saluta("Bob", "Buongiorno");  // usa il valore specificato e stampa: Buongiorno, Bob!
    return 0;
}
```
