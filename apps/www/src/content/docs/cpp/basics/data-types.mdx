---
title: Tipi di dati
description: "I tipi di dati fondamentali del C++: int, float, double, char e bool."
---

## Perché esistono i tipi di dati?

Pensa ai tipi come a scomparti destinati a valori diversi: uno può contenere interi, un altro caratteri o numeri decimali.

In C++ funziona allo stesso modo: ogni variabile ha un **tipo** che stabilisce che genere di valore può contenere. A differenza di Python, non puoi mettere qualsiasi cosa in qualsiasi variabile: devi dichiarare il tipo in anticipo.

Il compilatore può controllare meglio i valori e le operazioni compatibili con ogni variabile.

## Numeri interi: `int`

`int` (abbreviazione di _integer_, intero) contiene numeri senza virgola, sia positivi che negativi:

```cpp
int eta = 16;
int temperatura = -5;
int anno = 2024;
int zero = 0;
```

Su molti sistemi un `int` occupa 4 byte e contiene valori da circa -2 miliardi a +2 miliardi. Lo standard C++ non impone però una dimensione identica su ogni piattaforma. Per i primi programmi puoi usare `int` per i normali numeri interi.

Se hai bisogno di numeri molto grandi o molto piccoli, esistono varianti:

```cpp
short int piccolo = 32000;
long long enorme = 9000000000000LL;
```

## Numeri decimali: `float` e `double`

Per numeri con la virgola (come 3.14 o 1.75) hai due opzioni.

### `float`

Su molti sistemi occupa 4 byte e conserva circa 6-7 cifre significative. Metti la `f` alla fine del numero per indicare che il valore è un `float`:

```cpp
float temperatura = 36.6f;
float pi = 3.14159f;
```

### `double`

Su molti sistemi occupa 8 byte e conserva circa 15-16 cifre significative. È la scelta più comune per i calcoli decimali generici:

```cpp
double altezza = 1.75;
double pi = 3.14159265358979;
```

### Quale usare?

Per iniziare, preferisci `double` a meno che un progetto richieda espressamente `float`. Offre più precisione, ma i numeri decimali binari restano approssimazioni:

```cpp
// Possibile problema con float
float a = 0.1f + 0.2f;
cout << a;  // potrebbe stampare 0.3000001 invece di 0.3

// Anche double può mostrare un'approssimazione, ma conserva più cifre
double b = 0.1 + 0.2;
cout << b;
```

## Singolo carattere: `char`

`char` (abbreviazione di _character_, carattere) contiene un singolo carattere. I caratteri si scrivono tra **apici singoli** `'...'`:

```cpp
char lettera = 'A';
char cifra = '7';
char spazio = ' ';
```

Internamente, un `char` contiene un piccolo valore numerico. Per i caratteri di base, come `'A'`, quel valore segue normalmente la codifica ASCII. Un singolo `char` non basta invece a rappresentare in modo generale tutti i caratteri Unicode.

```cpp
char c = 'A';
cout << c;        // stampa: A
cout << (int)c;   // stampa: 65 (il codice numerico di 'A')
```

## Vero o falso: `bool`

`bool` (abbreviazione di _boolean_, booleano) può contenere solo due valori: `true` (vero) o `false` (falso):

```cpp
bool maggiorenne = true;
bool haPatente = false;
bool risultato = (5 > 3);   // true, perché 5 è maggiore di 3
```

Quando stampi un `bool`, di default viene mostrato come `1` (true) o `0` (false). Per stampare le parole `true` e `false`:

```cpp
bool x = true;
cout << x;               // stampa: 1
cout << boolalpha << x;  // stampa: true
```

## Quanto spazio occupano?

Puoi controllare quanti byte occupa un tipo con `sizeof`:

```cpp
cout << sizeof(int) << endl;    // 4
cout << sizeof(double) << endl; // 8
cout << sizeof(char) << endl;   // 1
cout << sizeof(bool) << endl;   // 1
```

## Riepilogo

| Tipo     | Spazio | Valori                                      |
| -------- | ------ | ------------------------------------------- |
| `int`    | 4 byte | numeri interi, da -2 miliardi a +2 miliardi |
| `float`  | 4 byte | decimali, ~7 cifre significative            |
| `double` | 8 byte | decimali, ~15 cifre significative           |
| `char`   | 1 byte | un singolo carattere                        |
| `bool`   | 1 byte | solo `true` oppure `false`                  |

## Convertire tra tipi

A volte hai bisogno di convertire un valore da un tipo all'altro.

La **conversione implicita** avviene automaticamente quando è sicura:

```cpp
int x = 5;
double y = x;   // int viene convertito in double automaticamente
cout << y;      // 5.0
```

La **conversione esplicita** (chiamata _casting_) si usa quando vuoi forzare una conversione:

```cpp
double pi = 3.14159;
int parteIntera = (int)pi;  // la parte decimale viene scartata
cout << parteIntera;        // 3 (non arrotondato, solo troncato)
```

Attenzione alla **divisione tra interi**: quando dividi due `int`, il risultato è sempre un intero (la parte decimale viene persa):

```cpp
int x = 5, y = 2;
int risultato = x / y;         // risultato = 2, non 2.5!
double risultato2 = (double)x / y;  // risultato2 = 2.5 (corretto)
```

## Esempio pratico

```cpp
#include <iostream>
using namespace std;

int main() {
    // Una variabile per ogni tipo principale
    int eta = 16;
    double altezza = 1.75;
    char iniziale = 'A';
    bool maggiorenne = false;

    cout << "Età (int): " << eta << endl;
    cout << "Altezza (double): " << altezza << endl;
    cout << "Iniziale (char): " << iniziale << endl;
    cout << "Maggiorenne (bool): " << boolalpha << maggiorenne << endl;

    return 0;
}
```

Output:

```
Età (int): 16
Altezza (double): 1.75
Iniziale (char): A
Maggiorenne (bool): false
```
