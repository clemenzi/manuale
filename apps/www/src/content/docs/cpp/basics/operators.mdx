...
Gli **operatori** permettono al programma di calcolare, confrontare valori e combinare condizioni.

Alcuni coincidono con i simboli matematici (`+`, `-`, `*`, `/`); altri servono a confrontare o assegnare valori.
...
double risultato = static_cast<double>(x) / y;
cout << risultato << endl;       // 3.5
```

`static_cast<double>(x)` crea per il calcolo una versione decimale di `x`. In C++ moderno questa forma rende esplicito il tipo di conversione ed è preferibile al vecchio cast `(double)x`.
