...
Se il programma usa il numero pi greco in dieci punti, una modifica alla precisione richiederebbe dieci cambiamenti.

Anche un valore come `18`, ripetuto senza spiegazione, non dice se rappresenta un'età minima, un limite o altro.

Le **costanti** risolvono entrambi i problemi: assegni un nome al valore e lo definisci una sola volta.
...
Il compilatore impedisce così le modifiche accidentali.

## La convenzione dei nomi

In questa guida scriveremo i nomi delle costanti **tutto in maiuscolo**, con le parole separate da underscore. È una convenzione, non una regola del linguaggio, e permette di riconoscerle a colpo d'occhio:
...
Il preprocessore del C++ offre anche la direttiva `#define`, ereditata dal C:
...
Con `#define`, il **preprocessore** sostituisce ogni occorrenza di `PI` con `3.14159` prima della compilazione vera e propria. Non crea una variabile con un tipo: esegue una sostituzione di testo.
...
Per i valori costanti, in genere si preferisce `const`:
