...
A volte il numero di ripetizioni dipende dai dati. Per esempio, puoi chiedere un numero finché l'utente non ne inserisce uno valido.

Il ciclo `while` ripete un blocco **finché la condizione è vera** e si ferma quando diventa falsa.
...
Il `while` è adatto quando non sai quante ripetizioni serviranno, per esempio quando leggi input finché l'utente non decide di uscire:
