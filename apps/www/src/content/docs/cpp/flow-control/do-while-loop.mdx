...
Il `do...while` serve quando il blocco deve essere eseguito almeno una volta prima del controllo.

È utile, per esempio, per mostrare un menu e chiedere solo dopo se continuare.
...
Il `do...while` è adatto quando vuoi chiedere qualcosa all'utente almeno una volta e ripetere finché non inserisce un valore corretto:
...
Con un `while` normale dovresti leggere l'input prima del ciclo; qui la lettura compare una sola volta.
