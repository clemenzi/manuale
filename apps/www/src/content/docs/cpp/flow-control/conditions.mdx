...
Un programma può seguire strade diverse in base ai dati: per esempio, mostrare un contenuto a un maggiorenne o accettare un voto sufficiente.

Le condizioni permettono di scegliere quale blocco eseguire.
...
Puoi mettere un `if` dentro un altro `if`, controllando prima la condizione esterna e poi quella interna.
...
Quando devi confrontare una variabile con molti valori fissi, `switch` può essere più leggibile di una lunga serie di `else if`.
