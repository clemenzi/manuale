...
Un array normale ha una dimensione fissata prima dell'esecuzione. Questo è scomodo quando il numero di dati non è noto.

Il `vector` è una sequenza ridimensionabile: **cambia dimensione automaticamente** quando aggiungi o rimuovi elementi.
...
Per controllare i limiti durante l'accesso, usa `.at()`:
...
cout << v.empty() << endl;    // false (0), perché non è vuoto
