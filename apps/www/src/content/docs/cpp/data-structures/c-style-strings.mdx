...
In C++ puoi rappresentare il testo con array di caratteri, ereditati dal C, oppure con `std::string`.

Conoscere entrambi serve per leggere codice esistente; nei nuovi programmi si usa quasi sempre `std::string`.
...
**Regola pratica: usa `std::string`.** Le stringhe C-style servono soprattutto per interoperare con codice C o API che le richiedono.
...
`std::string` offre concatenazione, confronto e gestione della lunghezza senza ricorrere alle funzioni di `<cstring>`:
...
In questo esempio `std::string` evita la gestione manuale dei buffer.
