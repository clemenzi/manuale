...
Molti programmi devono memorizzare liste, associare chiavi a valori o gestire code. La STL fornisce componenti per questi casi comuni.

La **STL** (Standard Template Library) è l'insieme di contenitori, algoritmi e iteratori della libreria standard del C++.
...
### `map`: il dizionario (chiave → valore)
...
### `unordered_map`: il dizionario veloce
...
### `stack`: la pila (ultimo entrato, primo uscito)
...
### `queue`: la coda (primo entrato, primo uscito)
