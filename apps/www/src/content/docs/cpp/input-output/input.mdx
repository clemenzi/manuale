...
`cin` legge dalla tastiera i dati inseriti dall'utente, così il programma può lavorare con valori scelti durante l'esecuzione.

## Cos'è `cin`?

`cin` legge l'input dalla tastiera e, come `cout`, appartiene alla libreria `<iostream>`.
...
Quando il programma arriva a questa riga, aspetta l'input e salva il valore letto nella variabile `x`.
...
Per leggere tutta la riga, spazi compresi, usa `getline()`:
