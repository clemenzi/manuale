...
Il formato predefinito non è sempre adatto: un prezzo può richiedere due cifre decimali e una tabella colonne allineate.

La libreria `<iomanip>` contiene gli strumenti per controllare la visualizzazione dei dati:
...
`setw(n)` riserva almeno `n` posizioni per il prossimo valore. Se il valore è più corto, aggiunge spazi (a sinistra per l'allineamento predefinito):
