...
Il C++ include strumenti per funzioni matematiche, file, ordinamento e molte altre operazioni comuni.

Questa raccolta è la **libreria standard** (Standard Library). Le singole parti si usano con `#include`.

## Input e output con `<iostream>`

Serve per stampare a schermo e leggere dalla tastiera.
...
## Funzioni matematiche con `<cmath>`
...
## Numeri casuali e utilità con `<cstdlib>` e `<ctime>`
...
## Funzioni sui caratteri con `<cctype>`
...
## Algoritmi pronti con `<algorithm>`
...
## Valori massimi e minimi dei tipi con `<limits>`
