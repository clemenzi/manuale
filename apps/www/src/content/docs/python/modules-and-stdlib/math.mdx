...
Per usarlo, importa il modulo all'inizio del programma:
...
Il modulo `math` include alcune costanti matematiche già pronte:
...
Il valore assoluto è la "distanza da zero" e non conserva il segno negativo.
...
Le funzioni trigonometriche lavorano in **radianti**, non in gradi. Python offre funzioni per convertire le due unità:
...
print(math.factorial(5))   # 120: fattoriale, cioè 5! = 5×4×3×2×1
```

## Il modulo `random`: numeri casuali

Usato spesso insieme a `math`, il modulo `random` genera numeri casuali per giochi e simulazioni:
