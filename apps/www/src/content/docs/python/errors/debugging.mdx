...
È normale trovare errori anche dopo aver scritto il codice. Il **debugging** (o "debug") consiste nell'individuarli e correggerli.

Quando qualcosa non funziona, raccogli informazioni, formula un'ipotesi e verifica se spiega il comportamento osservato.
...
Sono errori nel modo in cui hai scritto l'istruzione. Python li segnala **prima che il programma parta**.
...
Di solito Python indica il punto in cui ha trovato il problema.

---

### 2. Errori durante l'esecuzione

Il codice è valido, ma qualcosa va storto **durante l'esecuzione**.
...
L'indice richiesto non esiste perché la lista contiene solo tre elementi.

---

### 3. Errori logici

In questo caso il programma **gira senza errori**, ma produce risultati sbagliati. Devi confrontare il risultato con quello atteso.
...
Quando trova un errore, Python stampa un **traceback**: mostra le chiamate che hanno portato al punto dell'errore.
...
Per controllare i valori durante l'esecuzione puoi inserire temporaneamente alcune `print()`.
...
`pdb` permette di **mettere in pausa** il programma durante l'esecuzione e ispezionarne lo stato.
