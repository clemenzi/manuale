...
## A cosa servono le librerie esterne

La comunità Python ha creato librerie per molti problemi comuni. Puoi usarle invece di implementare ogni funzione da zero.

Esistono librerie per creare siti web, lavorare con l'intelligenza artificiale, analizzare dati e sviluppare videogiochi.

## Come usare una libreria esterna

Il processo consiste in tre passaggi:

**Passo 1: Installa** la libreria dal terminale

```bash
pip install requests
```

**Passo 2: Importa** la libreria nel codice
...
- **`matplotlib`**: crea grafici come istogrammi, torte, linee e dispersioni.
...
3. Guarda le stelle su GitHub: più stelle significa più popolare e affidabile
...
Prima di scrivere una funzione, controlla se la libreria standard o una libreria esterna offre già ciò che ti serve.
