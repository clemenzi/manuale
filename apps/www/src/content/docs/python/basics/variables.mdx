---
title: Variabili in Python
description: Come conservare e usare informazioni nel tuo programma Python.
---

## Cos'è una variabile?

Pensa a una serie di scatole con un'etichetta. Ogni etichetta ti permette di ritrovare il contenuto associato.

Come prima approssimazione, una **variabile** funziona così:

- L'etichetta è il **nome** della variabile
- Il contenuto della scatola è il suo **valore**
- La scatola rappresenta il valore che il programma può ritrovare attraverso quel nome

L'analogia non è perfetta: in Python una variabile è più precisamente un nome collegato a un oggetto. Più nomi possono anche riferirsi allo stesso oggetto. Per i primi esempi, però, l'idea dell'etichetta aiuta a seguire assegnazioni e modifiche.

## Creare una variabile

In Python crei una variabile scegliendo un nome e assegnandole un valore con il simbolo `=`.

```python
nome = "Alice"    # una scatola chiamata "nome" con dentro il testo "Alice"
eta = 16          # una scatola chiamata "eta" con dentro il numero 16
altezza = 1.65    # una scatola chiamata "altezza" con dentro il numero 1.65
```

Non devi dichiarare il tipo di dato: Python lo ricava dal valore. Dopo l'assegnazione, la variabile è disponibile nel programma.

## Usare una variabile

Una volta creata, puoi usare il nome della variabile ogni volta che ti serve il suo valore:

```python
nome = "Alice"
print(nome)              # Alice
print("Ciao, " + nome + "!")  # Ciao, Alice!
```

## Cambiare il valore

Puoi cambiare il valore di una variabile in qualsiasi momento:

```python
x = 10
print(x)   # 10
x = 25
print(x)   # 25
```

Puoi anche usare il vecchio valore per calcolarne uno nuovo:

```python
punteggio = 100
punteggio = punteggio + 50  # oppure: punteggio += 50
print(punteggio)             # 150
```

## Assegnare più variabili in una volta

Puoi creare più variabili in una sola riga:

```python
a, b, c = 1, 2, 3
print(a)  # 1
print(b)  # 2
print(c)  # 3
```

Oppure puoi assegnare lo stesso valore a più variabili contemporaneamente:

```python
x = y = z = 0
print(x, y, z)  # 0 0 0
```

## Regole per scegliere i nomi

I nomi delle variabili devono rispettare queste regole:

- Il nome deve iniziare con una lettera (`a-z`, `A-Z`) oppure con il trattino basso `_`
- Può contenere lettere, numeri e trattini bassi
- **Non può contenere spazi** (usa il trattino basso al loro posto)
- **Non può iniziare con un numero**
- **Non può essere una parola riservata** di Python (come `if`, `for`, `print`, ecc.)

```python
# Nomi validi ✓
nome = "Alice"
eta_utente = 16
_privato = 42
valore2 = 3.14

# Nomi non validi ✗
# 2variabile = 10    — inizia con un numero
# mio nome = "Bob"  — contiene uno spazio
# if = "A"          — è una parola riservata di Python
```

## Lo stile: snake_case

Per convenzione, i nomi delle variabili in Python usano lo **snake_case**: tutto minuscolo, con il trattino basso `_` tra le parole.

```python
# Stile consigliato ✓
nome_utente = "Alice"
eta_minima = 18
numero_di_studenti = 30

# Stile sconsigliato (funziona, ma non è la norma)
NomeUtente = "Alice"   # CamelCase, si usa per le classi
ETAMINIMA = 18         # tutto maiuscolo, si usa per le costanti
```

## Scoprire il tipo di una variabile

Puoi chiedere a Python che tipo di dato contiene una variabile con la funzione `type()`:

```python
x = 42
print(type(x))      # <class 'int'>  — numero intero

y = "ciao"
print(type(y))      # <class 'str'>  — stringa di testo

z = 3.14
print(type(z))      # <class 'float'>  — numero decimale
```

Questo torna utile quando non sei sicuro di cosa contiene una variabile.
