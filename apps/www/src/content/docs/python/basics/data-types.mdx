...
Per questo ogni valore in Python ha un **tipo di dato**: indica com'è fatto e quali operazioni puoi eseguire su di esso.
...
Il termine "stringa" deriva dall'inglese _string_ (filo), perché è come una sequenza di caratteri infilati uno dopo l'altro.
...
`None` è un valore speciale che rappresenta l'assenza intenzionale di un risultato o di un dato disponibile.
...
`None` è comunque un valore vero e proprio, di tipo `NoneType`. Non equivale a `0`, a `False` o a una stringa vuota.

## Tipi per raccogliere più valori insieme

Python ha anche tipi che raccolgono più valori. Li vedrai nei capitoli successivi; per ora basta riconoscerli:
...
In Python non devi dichiarare il tipo di una variabile: viene ricavato dal valore assegnato.
...
Questa caratteristica si chiama **tipizzazione dinamica**. Il tipo non è fisso e può cambiare durante il programma, anche se di solito è meglio evitarlo.
