...
In programmazione si conta quasi sempre da 0. È una convenzione che incontrerai spesso. `range(5)` genera 5 numeri, partendo da 0.
:::
...
:::tip
Nella maggior parte dei casi, è più semplice e leggibile usare `enumerate()` invece di `range(len(...))`. Vedi il capitolo sui cicli `for`.
:::
