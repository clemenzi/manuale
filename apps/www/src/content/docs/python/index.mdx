...
Immagina un cuoco che segue una ricetta passo dopo passo. Non prepara tutto in anticipo: legge una riga, fa quell'azione, poi passa alla riga successiva.

Python funziona in modo simile: è un linguaggio **interpretato**. Significa che scrivi il codice, e un programma chiamato **interprete** lo legge ed esegue riga per riga, immediatamente.
...
1. Scrivi il codice in un file con estensione `.py` (un file è come una pagina di quaderno salvata nel computer)
2. L'interprete Python legge ed esegue il file
3. Vedi il risultato nel terminale (una finestra dove scrivi istruzioni per il computer e leggi le sue risposte)
...
Durante l'installazione su Windows, assicurati di spuntare la casella **"Add Python to PATH"**. Pensa al PATH come a una lista di indirizzi: quando scrivi `python`, il computer guarda in quella lista per trovare lo strumento giusto. Se Python non è nella lista, il terminale non sa dove cercarlo.
...
Ecco come appare la console interattiva quando provi dei comandi:
