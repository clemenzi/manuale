...
## Tecniche avanzate per i dizionari

Conosci già i dizionari di base. Queste tecniche permettono di **crearli in modo più compatto**, gestire chiavi mancanti e contare le occorrenze.

## Dictionary comprehension: creare dizionari in una riga

Come le list comprehension, le dictionary comprehension costruiscono una struttura in una sola espressione; il risultato è un dizionario:
...
Se accedi a una chiave inesistente con `dizionario["chiave"]`, Python solleva un `KeyError`. Il metodo `get()` restituisce invece un valore predefinito.
...
`setdefault()` restituisce il valore esistente; se la chiave manca, la aggiunge con il valore predefinito e restituisce quest'ultimo.
