...
Conosci già le liste di base. Qui trovi tecniche per scrivere codice Python **più corto, più leggibile e più efficiente**.

---

## List comprehension: creare liste in una riga

La **list comprehension** costruisce una lista a partire da un'altra sequenza in una sola riga.
...
Leggi la list comprehension come: _"x al quadrato, per ogni x in range(1, 6)"_.
...
:::note
Spesso le list comprehension sono più leggibili di `map` e `filter`. Usa quella forma che trovi più chiara.
:::
