...
I dati grezzi non sono sempre facili da leggere. La formattazione permette di scegliere il numero di cifre decimali e di allineare il testo in colonne.

## Le f-string: il metodo consigliato

Le **f-string**, disponibili da Python 3.6, permettono di inserire variabili nelle stringhe in modo diretto. Si scrivono mettendo `f` prima delle virgolette:
...
Tra le parentesi graffe puoi scrivere un'espressione Python:
...
Con questo formato Python moltiplica per 100 e aggiunge automaticamente `%`.
...
Per creare colonne allineate puoi specificare larghezza e direzione dell'allineamento:
...
**`sep`**: il testo che Python mette tra gli argomenti (default: uno spazio).
