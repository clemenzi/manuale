...
Quando un programma si chiude, i dati presenti solo in memoria vengono persi. Per ritrovarli alla successiva esecuzione, devi **salvarli su file**.

## Aprire un file

Per lavorare con un file usa `open()`, indicando il nome e la **modalità** di apertura:
...
Usa il costrutto `with` per aprire i file. Li chiude correttamente anche se nel blocco si verifica un errore:
...
Se apri in modalità `"r"` un file inesistente, Python solleva un errore. Puoi gestirlo con `try/except`:
